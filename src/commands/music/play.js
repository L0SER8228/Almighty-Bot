module.exports = {
  ...
  async execute(bot, interaction) {
    if (!bot.utils.havePermissions(interaction))
      return bot.say.wrongMessage(interaction, "I need the following permission \`View Channel\`, \`Send Messages\`, \`Embed Links\`.");
    ...
    if (guildQueue) {
      ...
      if (!channel.joinable)
        return bot.say.wrongMessage(interaction, "I need \`Connect Channel\` permission.");
    ...
    if (!result || !result.tracks.length)
      return bot.say.wrongMessage(interaction, `No result was found for \`${string}\`.`);
    ...
    try {
      ...
      return bot.say.wrongMessage(interaction, `Could not join your voice channel!\n\`${error}\``);
    }
